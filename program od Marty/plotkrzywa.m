...
switch handles.viewmode
    case 1  % view both
        ...
        plot_data=[krzywe.F_do(i,:); krzywe.F_od(i,:)]';
        pcol(1)='r';
        pcol(2)='b';
        
        if (handles.reference)
            z_data=[krzywe.z_do(i,:); krzywe.z_od(i,:);krzywe.z_do(j,:); krzywe.z_od(j,:)]';
            plot_data=[krzywe.F_do(i,:) ; krzywe.F_od(i,:);krzywe.F_do(j,:); krzywe.F_od(j,:)]';
            pcol(1)='r';
            pcol(2)='b';
            pcol(3)='k';
            pcol(4)='g';
    ...
    case 2   % view approach
        ...
        plot_data=[krzywe.F_do(i,:)];
        pcol(1)='r';
        if (handles.reference)
            z_data=[krzywe.z_do(i,:);krzywe.z_do(j,:)]';
            plot_data=[krzywe.F_do(i,:) ;krzywe.F_do(j,:) ];
            pcol(1)='r';
            pcol(2)='k';
        end    
    case 3   % view retract 
        z_data=[krzywe.z_od(i,:)]';
        plot_data=[krzywe.F_od(i,:) ];
        myColororder=[0 0 0];
        pcol(1)='b';
        if (handles.reference)
            z_data=[krzywe.z_od(i,:);krzywe.z_od(j,:)]';
            plot_data=[krzywe.F_od(i,:) ;krzywe.F_od(j,:) ]';
            pcol(1)='b';
            pcol(2)='g';    
        end
end
