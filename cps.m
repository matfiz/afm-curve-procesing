...
function read_stress_relaxation(hObject, curve_number)
%read handles
handles = guidata(hObject);
%read relaxation handles
    relaxationPanel = findobj('tag','cps_stress_relaxation');
    ...
    handlesRelaxation = guidata(relaxationPanel);
%get relaxation info
curve = handles.curves(curve_number);
if curve.mode == 'constant-height'
    if curve.hasStressRelaxation == true
        fit_stress_relaxation(relaxationPanel,hObject,curve_number); 
        handlesRelaxation = guidata(relaxationPanel);
        set(handlesRelaxation.checkbox_relaxation,'Value',1);
        set(handlesRelaxation.table_relaxation,'Visible','on');
        set(handlesRelaxation.slider_relaxation,'Visible','on');
        set(handlesRelaxation.slider_relaxation,'Max',handles.current_curve.pauseLength);
        set(handlesRelaxation.slider_relaxation,'Value',handles.curves(curve_number).StressRelaxationFitLength);
        set(handlesRelaxation.f_relaxation_fit_range,'String',num2str(handles.curves(curve_number).StressRelaxationFitLength/handles.current_curve.pauseLength*100,'%5.0f%%'));
    else
        set(handlesRelaxation.checkbox_relaxation,'Value',0);
        set(handlesRelaxation.table_relaxation,'Visible','off');
        set(handlesRelaxation.slider_relaxation,'Visible','off');
    end
end
    
%save relaxation handles
guidata(relaxationPanel, handlesRelaxation);
