...
function write_elasticity(hObject)
    ...
    if ~handles.header_written
        fprintf(f,'%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n',...
            ...
            'CP position [nm]');
        %set table headers
        set(handles.t_output,'ColumnFormat',{'char' 'char' 'numeric' 'numeric' 'numeric' 'numeric' 'numeric' 'numeric'});
        set(handles.t_output,'ColumnName',{'Name' 'Model' 'Radius [nm]' 'E [Pa]' 'b' 'Skip initial' 'Shift CP' 'CP position [nm]'});
        handles.header_written = true;
    end
    ...
    if curve.hasElasticityFit
        ...
        fprintf(f,'%s\t%s\t%f\t%f\t%f\t%f\t%f\t%f\n',...
            ...
            elasticityParams.b, elasticityParams.excludeInitial,...
